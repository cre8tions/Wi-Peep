...
class Handler:
    ...
    def snarf(self, q):
        ...
        sniff(iface = dev_from_index(30), prn = lambda x: q.put(x), store = 0)


    def sniffQueue(self):
        """Sets up the queue for sniffing"""
        self.q = Queue()
        sniffer = threading.Thread(target = self.snarf, args = (self.q,))
        sniffer.daemon = True
        sniffer.start()
...
#                if x.type == 0:
                if pkt.haslayer(Dot11Beacon):
                    ...
                    self.beaconMirror(pkt, q.qsize(), i)
                    time.sleep(self.args.threadSleep)
                elif pkt.haslayer(Dot11QoS):
                    if pkt.addr1 == pkt.addr3:
#                        if x.subtype == 12:
                        if self.dsDict.get(pkt.addr3) is not None:
                            if self.verbose is True:
                                print(f'polo  - {pkt.addr2:17} {self.dsDict.get(pkt.addr3)[0].decode():20} {pkt.addr3} {pkt.dBm_AntSignal:4} {pkt.time}')
...
if __name__ == '__main__':
    ...
    parser.add_argument('-b', help = 'comma delimited bssid targets', metavar = '<comma delimited bssid targets>')
    parser.add_argument('-i', help = 'Injection NIC', metavar = '<inj nic>', required = True)
    parser.add_argument('-m', help = 'Monitor NIC', metavar = '<mon nic>', required = True)
